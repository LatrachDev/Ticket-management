                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($tickets as $ticket)
                                <tr>
                                    ...
                                    <td class="px-6 py-4">{{ Str::limit($ticket->description, 100) }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('tickets.assign', $ticket) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <select name="agent_id" onchange="this.form.submit()" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
